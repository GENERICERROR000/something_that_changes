...
function preload() {
	...
	// load font
	clockFont = loadFont("digital_7_mono.ttf")
}
...
function setup() {
	...
	clockCanvas = createGraphics(windowWidth, windowHeight, WEBGL)

	// context (image) to be passed to shader
	textCanvas = createGraphics(windowWidth, windowHeight, WEBGL)

	// fix for shader rendering flipped across y axis
	textCanvas.scale(-1, 1)
...
function clock() {
	...
	textCanvas.textFont(clockFont)
	textCanvas.textAlign(CENTER, CENTER)
	textCanvas.textSize(width / 6)

	// text alpha 
	textCanvas.fill(255, 0, 0, CLOCK_ALPHA)

	// text rotation
	textCanvas.push()
	textCanvas.rotate(CLOCK_ROTATION)

	// get time
	let time = getTime()

	// write text to textCanvas
	textCanvas.text(time, CLOCK_X, CLOCK_Y)
	textCanvas.pop()

	// create geometry for text to render on
	clockCanvas.rect(0, 0, windowWidth, windowHeight)

	// render clockCanvas to main canvas
	image(clockCanvas, 0, 0, windowWidth, windowHeight)
}
...
function blurFace() {
	...
	// load shader to clockCanvas
	clockCanvas.shader(blurShader)

	// pass text canvas as image to shader
	// (will be rendered on clockCanvas)
	blurShader.setUniform("tex0", textCanvas)
